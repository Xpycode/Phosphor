...
Width:   [____640____] px
            🔗              ← Toggle (bright=locked, dim=unlocked)
...
### Files Modified
- `Models/ExportSettings.swift` — aspect ratio properties and methods (~25 lines)
- `Views/Settings/ResizeSection.swift` — lock toggle UI and bindings (~80 lines)

### Git
- Commit: `507f0f8` — "feat(canvas): add aspect ratio lock for Custom mode"
